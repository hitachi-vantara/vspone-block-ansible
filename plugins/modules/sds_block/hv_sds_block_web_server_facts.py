...
DOCUMENTATION = """
---
module: hv_sds_block_web_server_facts
short_description: Get the web server access setting from VSP One SDS Block and Cloud systems.
description:
  - Get the web server access setting from VSP One SDS Block and Cloud systems.
  - For examples, go to URL
    U(https://github.com/hitachi-vantara/vspone-block-ansible/blob/main/playbooks/sds_block_direct/sdsb_web_server_facts.yml)
version_added: "4.5.0"
author:
...
EXAMPLES = """
- name: Retrieve information about all web server settings
  hitachivantara.vspone_block.sds_block.hv_sds_block_web_server_facts:
    connection_info:
...
ansible_facts:
  description: >
    Dictionary containing web server access settings discovered from the system.
  returned: always
  type: dict
  contains:
    web_server_setting:
...
class SDSBWebServerFactsManager:
    def __init__(self):
        ...
        self.logger = Log()
        self.argument_spec = (
            WebServerAccessSettingsArgs().web_server_access_settings_facts()
        )
    ...
    def apply(self):
        ...
        try:
            ...
            web_server_settings = sdsb_reconciler.web_server_settings_facts()
        ...
        data = {"web_server_setting": web_server_settings}
