...
---
module: hv_hg_bulk
short_description: Manages host groups in bulk on VSP block storage system with parallel processing.
description:
  - This module provides bulk host group management operations with multithreading support for parallel processing across multiple ports.
  - Operations on multiple ports are executed concurrently using a thread pool (up to 10 concurrent threads) for improved performance.
  - Supported operations include
  - 1. create host group on multiple ports in parallel.
  - 2. delete host group from multiple ports in parallel.
  - 3. add logical unit to host group across multiple ports.
  - 4. remove logical unit from host group across multiple ports.
  - 5. add host WWN to host group across multiple ports.
  - 6. remove host WWN from host group across multiple ports.
  - 7. set host mode across multiple ports.
  - 8. add host mode option to host group across multiple ports.
  - 9. remove host mode option from host group across multiple ports.
  - Thread-safe result aggregation ensures accurate reporting of changes, comments, and errors from all parallel operations.
    U(https://github.com/hitachi-vantara/vspone-block-ansible/blob/main/playbooks/vsp_direct/hostgroup_bulk.yml)
version_added: '4.7.0'
author:
...
EXAMPLES = """
- name: Create host group on multiple ports in parallel
...
- name: Delete host group from multiple ports in parallel
  hitachivantara.vspone_block.vsp.hv_hg_bulk:
    state: absent
    connection_info:
      address: storage1.company.com
...
- name: Add WWNs to host groups across multiple ports in parallel
  hitachivantara.vspone_block.vsp.hv_hg_bulk:
    state: added
    connection_info:
      address: storage1.company.com
...
- name: Remove WWNs from host groups across multiple ports in parallel
  hitachivantara.vspone_block.vsp.hv_hg_bulk:
    state: removed
    connection_info:
      address: storage1.company.com
...
- name: Update host mode on multiple ports in parallel
  hitachivantara.vspone_block.vsp.hv_hg_bulk:
    state: updated
    connection_info:
      address: storage1.company.com
...
- name: Add LUN paths to host groups across multiple ports in parallel
  hitachivantara.vspone_block.vsp.hv_hg_bulk:
    state: added
    connection_info:
      address: storage1.company.com
...
class VSPBulkHostGroupManager:
    def __init__(self):
        self.logger = Log()
        self.argument_spec = VSPHostGroupArguments().host_group_bulk()
    ...
    def apply(self):
        self.logger.writeInfo("=== Start of Host Group Bulk operations ===")
        ...
        try:
            ...
            if host_group_data.get("failed"):
                msg_string = "; ".join(host_group_data.get("errors", []))
                self.module.fail_json(
                    msg=f"Host group bulk operation failed: {msg_string}",
                    **host_group_data,
                )
        except Exception as e:
            self.logger.writeError(str(e))
            self.logger.writeInfo("=== End of Host Group Bulk operations ===")
            self.module.fail_json(msg=str(e))
        if registration_message:
            host_group_data["user_consent_required"] = registration_message
        self.logger.writeInfo("=== End of Host Group Bulk operations ===")
        self.module.exit_json(**host_group_data)
    ...
    def direct_host_group_modification(self):
        ...
        ).host_group_bulk_reconcile(self.state, self.spec)
        if result is None:
...
def main():
    obj_store = VSPBulkHostGroupManager()
    obj_store.apply()
