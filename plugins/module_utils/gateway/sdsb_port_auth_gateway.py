...
class SDSBPortAuthDirectGateway:
    ...
    def get_port_by_name(self, port_name):
        ...
        logger.writeDebug(
            "GW:get_port_by_name:port_data={}", compute_ports_data
        )
    ...
    def remove_chap_user_access_from_port(self, port_id, chap_user_id):
        end_point = SDSBlockEndpoints.DELETE_PORT_AUTH_SETTINGS_CHAP_USERS.format(port_id, chap_user_id)
        chap_user_data = self.connection_manager.delete(end_point)
        logger.writeDebug("GW:remove_chap_user_access_from_port:data={}", chap_user_data)
        return chap_user_data            
