    ...
    def get_volumes(self, get_volume_spec: VolumeFactSpec):
        ...
        if get_volume_spec.lun:
            return VSPVolumesInfo(
                data=[self.provisioner.get_volume_by_ldev(get_volume_spec.lun)]
            )
    ...
    def extract(self, responses):
        ...
        for response in responses:
            ...
            if response.get("isDataReductionShareEnabled") is not None:
                new_dict["is_data_reduction_share_enabled"] = response.get(
                    "isDataReductionShareEnabled"
                )
            for key, value_type in self.common_properties.items():
                ...
                if response_key or isinstance(response_key, int):
                    ...
                    new_dict[key] = (
                        response_key
                        if response_key
                        else volume_id_to_hex_format(response.get("ldevId"))
                    )
                else:
