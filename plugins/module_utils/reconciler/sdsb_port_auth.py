...
class SDSBPortAuthReconciler:
        ...
        if state.lower() == StateValue.PRESENT:
            ...
            if spec.authentication_mode is not None:
                ...
                if auth_mode is None:
                    raise ValueError(SDSBPortAuthValidationMsg.INVALID_AUTH_MODE.value.format(spec.authentication_mode))
            ...
            if port is None:
                raise ValueError(SDSBPortAuthValidationMsg.PORT_NOT_FOUND.value.format(spec.port_name))
            port_id = port.id
            

            if spec.target_chap_users is not None and len(spec.target_chap_users) > 0:
                logger.writeDebug("RC:reconcile_port_auth:target_chap_users = {}", spec.target_chap_users)
                if spec.target_chap_users[0] == None:
                   raise ValueError(SDSBPortAuthValidationMsg.INVALID_CHAP_USER_LIST.value) 
                if spec.state is None:
                    ...
                    if spec.state.lower() == SDSBPortAuthSubstates.ADD_CHAP_USER:
                        ...
                        raise ValueError(SDSBPortAuthSubstates.INVALID_SPEC_STATE.value.format(
    ...
    def add_chap_user_to_port(self, port_id, target_chap_users):
        ...
        logger.writeDebug("RC:add_chap_user_to_port:all_chap_users = {}", all_chap_users)
        ...
        logger.writeDebug("RC:add_chap_user_to_port:target_chaps_dict = {}", target_chaps_dict) 
        ...
        logger.writeDebug("RC:add_chap_user_to_port:port_chap_users = {}", port_chap_users)
        chap_ids_to_add = []
        ...
        logger.writeDebug("RC:add_chap_user_to_port:chap_ids_to_add = {}", chap_ids_to_add)
    ...
    def remove_chap_user_access_from_port(self, port_id, chap_ids_to_remove):
        logger.writeDebug("RC:remove_chap_user_access_from_port:chap_ids_to_remove = {}", chap_ids_to_remove)
    ...
    def update_sdsb_port_auth_settings(self, port_id, spec):
        ...
        logger.writeDebug("RC:update_sdsb_port_auth_settings:port_auth_setting = {}", port_auth_setting)
        ...
        if spec.is_discovery_chap_authentication is None:
            ...
            if spec.is_discovery_chap_authentication != port_auth_setting.isDiscoveryChapAuth:
                changed = True
        ...
        if changed == True:
            ...
            self.update_port_auth_settings(port_id, auth_mode, is_discovery_chap_auth, is_mutual_chap_auth)

    @log_entry_exit
    def update_port_auth_settings(self, port_id, auth_mode, is_discovery_chap_auth, is_mutual_chap_auth):
        self.connection_info.changed = True
        self.provisioner.update_port_auth_settings(port_id, auth_mode, is_discovery_chap_auth, is_mutual_chap_auth)