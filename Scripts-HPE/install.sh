...
# else
  	/usr/bin/rpm -Uvh HPE_Storage_Ansible-2.3.0.1-1.el7.x86_64.rpm 
# fi
...
if [[ "$doAnsibleModule" -eq 1 ]]; then	
	...
	export HV_STORAGE_ANSIBLE_PATH=/opt/hpe/ansible
	...
	if [ -f /etc/profile.d/custom.sh ]; then
	    ...
	    if [ $? -ne 0 ]; then
	        echo "export HV_STORAGE_MGMT_PATH=/opt/hpe/ansible"  >>/etc/profile.d/custom.sh		
