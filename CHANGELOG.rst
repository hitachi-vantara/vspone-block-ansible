...
This release adds support for VSP One SDS Block version 1.19.00, SVOS 9.8.7 for VSP 5100/5500 &
VSP 5200/5600, SVOS 10.5.2 for VSP One B24/B26/B28, and Ansible core version 2.20. It introduces
new bulk modules for GAD, host groups, HUR, iSCSI targets, LDEVs, and TrueCopy, new automation
roles for GAD, HUR, quorum disk, and journal provisioning, and renames multiple input and output
parameters across many modules for improved consistency. Two deprecated playbooks have also been
removed.
...
- hitachivantara.vspone_block.vsp.hv_gad_bulk - Manages batch GAD pairs on VSP block storage systems.
- hitachivantara.vspone_block.vsp.hv_hg_bulk - Manages host groups in bulk on VSP block storage system with parallel processing.
- hitachivantara.vspone_block.vsp.hv_iscsi_target_bulk - Manages iscsi targets in bulk mode on VSP block storage systems.
- hitachivantara.vspone_block.vsp.hv_ldev_bulk - Manages multiple logical devices (LDEVs) in bulk on VSP block storage systems.
- hitachivantara.vspone_block.vsp.hv_truecopy_bulk - Manages bulk TrueCopy pairs on VSP block storage systems.
...
This release adds support for Mainframe z16 and SVOS 10.5.1, introduces multiple new modules for
VSP One SDS Block and Cloud systems (audit logging, event logs, journaling, licensing, login
messages, and external authentication), and expands capabilities across VSP block storage modules
including storage pools, LDEVs, snapshots, host groups, and facts modules.
...
- hitachivantara.vspone_block.sds_block.hv_sds_block_audit_log_setting - Manages Hitachi SDS block storage system audit log settings.
- hitachivantara.vspone_block.sds_block.hv_sds_block_audit_log_setting_facts - Get audit log setting from SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_dump_log_file - Dumps log information from SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_dump_log_status_facts - Dumps log status information from SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_event_log_setting - Manages Hitachi SDS block storage system event log settings.
- hitachivantara.vspone_block.sds_block.hv_sds_block_event_log_setting_facts - Get event log setting from SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_journal - Create and update Journals from storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_journal_facts - Retrieve journal information from storage SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_license - Manage licenses on SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_license_facts - Get license(s) from SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_license_setting - Manage license settings for SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_license_setting_facts - Get license setting from SDS Block storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_login_message - Update the login message displayed in the GUI login window and CLI warning banner.
- hitachivantara.vspone_block.sds_block.hv_sds_block_login_message_facts - Get the login message displayed in the GUI login window and CLI warning banner.
- hitachivantara.vspone_block.sds_block.hv_sds_block_storage_external_auth_server_setting - Manages external authentication server settings on VSP One SDS Block and Cloud systems.
- hitachivantara.vspone_block.sds_block.hv_sds_block_storage_external_auth_server_setting_facts - Get external authentication server settings from the storage system.
- hitachivantara.vspone_block.sds_block.hv_sds_block_storage_user_auth_setting - Manages external authentication server settings on VSP One SDS Block and Cloud systems.
- hitachivantara.vspone_block.sds_block.hv_sds_block_storage_user_auth_setting_facts - Get user authentication settings from the storage system.

Vsp
~~~

- hitachivantara.vspone_block.vsp.hv_pav_alias - Manages PAV (Parallel Access Volume) alias assignments for VSP block storage systems.
- hitachivantara.vspone_block.vsp.hv_pav_alias_facts - retrieves information about PAV aliases from VSP block storage systems.
- hitachivantara.vspone_block.vsp.hv_session - Manages sessions on VSP block storage systems.
- hitachivantara.vspone_block.vsp.hv_session_facts - Retrieves information about sessions on VSP block storage systems.
- hitachivantara.vspone_block.vsp.hv_snapshot_family_facts - Retrieves snapshot family information of the provided LDEV ID from VSP block storage systems.
- hitachivantara.vspone_block.vsp.hv_supported_host_mode_facts - Retrieves supported host mode options information from a specified VSP block storage system.
- hitachivantara.vspone_block.vsp.hv_vclone_parent_volume_facts - Retrieves virtual clone parent volume information from VSP block storage systems.
