...
class build_versioning():
    ...
    def _parse_version(self):
        ...
        """
        with open(self.inc_file, 'r') as istream:
        ...
        for line in data:
            ...
            if len(fields) != 2:
                continue
            if fields[0].startswith(self.inc_version[0]):
                version[0] = int(fields[1])
            elif fields[0].startswith(self.inc_version[1]):
                version[1] = int(fields[1])
            elif fields[0].startswith(self.inc_version[2]):
                version[2] = int(fields[1])
            elif fields[0].startswith(self.inc_version[3]):
    ...
    def _generate_inc_version(self, ofile, version):
        ...
        with open(ofile, 'w') as ostream:
            ...
            line = self.inc_version[-1] + '=' + buildno + '\n'
            ostream.write(line)
        ostream.close()
    ...
    def write_version(self, newversion):
        ...
        # Save new version
        self._generate_inc_version(self.inc_file, newversion)
