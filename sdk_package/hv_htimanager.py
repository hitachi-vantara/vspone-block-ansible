...
class HTIManager():
    ...
    def getHTIPair(self, lun, target_lun, mirror_id):
        logging.debug("mirror_id={0}".format(mirror_id))
        if mirror_id is None:
            pairs = [pair for pair in self.getHTIPairs(lun) if str(pair.get("SVol")) == str(target_lun)]
        else:
            pairs = [pair for pair in self.getHTIPairs(lun) if str(pair.get("MirrorId")) == str(mirror_id)]
