...
from ansible.module_utils.hv_storage_enum import StorageType, StorageModel
...
def getResourceGroupDriveGroups(storageSystem, rgId):
    
    result = ["5-5"]
#     resp = storageSystem.getResourceGroupDriveGroups(rgId)
#     for item in resp:
#         result.append(item["LunNumber"])
    return result

def getResourceGroupHostGroups(storageSystem, rgId):
    
    result = ["CL2-B-2", "CL2-B-3", "CL2-B-4"]
#     resp = storageSystem.getResourceGroupHostGroups(rgId)
#     for item in resp:
#         result.append(item["LunNumber"])
    return result

def getResourceGroupPorts(storageSystem, rgId):
    
    result = ["CL8-B"]
#     resp = storageSystem.getResourceGroupPorts(rgId)
#     for item in resp:
#         result.append(item["LunNumber"])
    return result

def getResourceGroupLogicalUnits(storageSystem, rgId):
    
#     result = [679,680,682]
    result = [679,680,681,682]
#     resp = storageSystem.getResourceGroupLogicalUnits(rgId)
#     for item in resp:
#         result.append(item["LunNumber"])
    return result
...
def getVSM(storage_serial, virtual_storage_serial):
        ...
        #storageSystem = StorageSystem(storage_serial).vsmManager
...
#             return None
        ...
        #vsms = storageSystem.getVSMBySerial(storage_serial)
        
        ## getVSM from the virtual_storage_serial perspective
        storageSystem = StorageSystem(virtual_storage_serial).vsmManager
        vsms = storageSystem.getVSMBySerial(virtual_storage_serial)        
        ...
        if vsms is None:
            ...
            if vsms.get("Serial") is not None:
                ...
                if str(vserial) == str(0):
                    ##FIXME: the serial=0 is a quick fix to indicate no result from getVSM,
                    ##it was throwing exception before and ansible will intercept
                    ##the exception before we can catch it here and handle it
        ...
        if vsms is not None: 
            vsmResult = vsms
#             ResourceGroups = vsms["ResourceGroups"]
#             
#             found=False
...
#                     rsrc["HostGroupGroups"] =   getResourceGroupHostGroups(     storageSystem,rsrc["ResourceGroupId"])
#                     rsrc["DriveGroups"] =       getResourceGroupDriveGroups(    storageSystem,rsrc["ResourceGroupId"])
#                     rsrc["Ports"] =             getResourceGroupPorts(          storageSystem,rsrc["ResourceGroupId"])
#                     rsrc["LogicalUnits"] =      getResourceGroupLogicalUnits(   storageSystem,rsrc["ResourceGroupId"])
                    ##writeNameValue("getResourceGroupItems, LogicalUnits={}", rsrc["LogicalUnits"])
#                     found=True
            
#             if found:
#                 vsmResult.append(vsms)
...
def validateMS(meta_resources):
    ...
    for resource in meta_resources:        
        if "serial" not in resource:
            raise Exception("Meta Resource object must include a serial number!! (Error at index {})".format(len(resource_groups)))
...
def getResourceGroup(vsm, storage_serial, virtual_storage_serial, name):
        ...
        #for vsm in vsms:
            ...
            rgs = []
            for rsrc in ResourceGroups:
                writeNameValue("getResourceGroup.MetaResourceSerial={}",rsrc["MetaResourceSerial"])
                writeNameValue("getResourceGroup.VirtualDeviceId={}",rsrc["VirtualDeviceId"])
                writeNameValue("getResourceGroup.ResourceGroupName={}",rsrc["ResourceGroupName"])
                if str(rsrc["MetaResourceSerial"]) == str(storage_serial) and str(rsrc["VirtualDeviceId"]) == str(virtual_storage_serial) and rsrc["ResourceGroupName"] == name:
                    rgs.append(rsrc)
                    found=True
...
def handleHostGroups(old, new, doAdd, storage, rgId):
    ...
    if doAdd:
        ...
        for hgPort in new:
            handleHostGroupsInSamePort(hgPort, hgMapOld[hgPort], hgMapNew[hgPort], doAdd, storage, rgId)         
    else:
...
def handleRemoveResourceGroup(vsms, playbook_meta_resources, playbook_virtual_storage_serial):
        ...
        for vsm in vsms:
            ...
            for rsrc in ResourceGroups:
                ...
                writeNameValue("storage_serial={}",storage_serial)
                writeNameValue("name={}",name)                
                rg = getResourceGroupInPlayBook(playbook_meta_resources, storage_serial, name)
                if rg is None:
                    writeMsg("sub.state=absent, ResourceGroup exists but not in playbook, doDelete")
...
def doUpdate(vsm, virtual_storage_serial, resource, resource_groups, doAdd, storageSystem, model):
        ...
        if rgs is None:
            if doAdd:
                writeMsg("no old resource, we need to add new resource group specified in the playbook")
                #doAddResourceGroup(storageSystem, storage_serial, model, name)
                doAddResourceGroup(storageSystem, virtual_storage_serial, model, name)
                ## FIXME: for some reason we are not getting 
                ## the json back from createVirtualBoxResourceGroup
                ## so we have to getVSM for now
                vsm = getVSM(None, virtual_storage_serial)
                rgs = getResourceGroup(vsm, storage_serial, virtual_storage_serial, name)
                ##TODO after add RG, could we make these call to add the resource? 
                ##we need to get the newly added rgId? call getVSM?
#                 handleDriveGroups(None, resource["parity_groups"], doAdd, storageSystem)
#                 handleHostGroups(None, resource["host_groups"], doAdd, storageSystem)
#                 handleLogicalUnits(None, resource["luns"], doAdd, storageSystem)
#                 handlePorts(None, resource["ports"], doAdd, storageSystem)
                
                ## its newly created, so rgs.size should only be 1
                for rsrc in rgs:                    
          ...
          #writeNameValue("doUpdate:after getResourceGroup rgs={}",rgs)
          for rsrc in rgs:                    
            ...
            ## playbook vars are optional
            item = resource.get("luns", None)
            handleLogicalUnits(getUnDefinedLuns(rsrc), rsrc["LogicalUnits"], item, doAdd, storageSystem, rgId)
...
def doCreateRGsForVsmCreate(resource, resource_groups):
        ...
        for hg in resource.get("host_groups", []):
            if "port" not in hg:
                raise Exception("Host Group object must include the port!! (Error at index {})".format(len(host_groups)))
            host_groups.append({"hostgroupIds": hg.get("host_group_names", []), "port": hg["port"]})

        new_resource = {
            "physicalSerialNumber": resource["serial"],
            "resourceGroupName": resource.get("name", resource["serial"]),
            "logicalUnits": resource.get("luns", []),
...
def testModel(): 
    model="VSP_F400"       
    model="VSP_G1000"       
    storage_type = StorageType.fromString(getTypeFromModel(model))
    storage_model = StorageModel.fromString(model)
    writeNameValue("storage_type={}",storage_type.value)
    writeNameValue("storage_model={}",storage_model.value)
           
def runPlaybook(module, isForceCreate):
    
        logger.writeEnterModule(moduleName)
        
        #testModel()
        #module.exit_json(changed = True, storageSystem = {})
    
        virtual_storage_serial = module.params["virtual_storage_serial"]
        physical_storage_serial = module.params.get("physical_storage_serial",None)
        storageSystem = StorageSystem(virtual_storage_serial).vsmManager
        ...
        if state != "absent":
            ...
            ##msg = "remove this debugging stop"
            ##module.exit_json(changed = False, comment = msg )            
...
#                 ## for each rg in the meta_resources of the playbook, do a targeted getVSM
#                 vsm = []
#                 if isForceCreate is False:
#                     vsm = getVSM(physicalSerialNumber, virtual_storage_serial)
                #writeNameValue("after getVSM:{}",vsm)
                if isForceCreate or vsm is None or len(vsm)==0: 
                    ...
                    writeMsg("do update")
                    vsm = doUpdate(vsm, virtual_storage_serial, resource, resource_groups, subobjState!="absent", physicalStorageSystem, model)
                    isCreate = False
            ...
            if subobjState == "absent" :
                ## walk thru the getVSM and see if any of the resource group
                ## is not in the playbook, if so remove it
                handleRemoveResourceGroup(result["vsm"], meta_resources, virtual_storage_serial)
            ...
            if isCreate:
                ...
                writeMsg("VSM already exists")
                result = {}
                result["comment"] = "VSM already exists"
        else:
