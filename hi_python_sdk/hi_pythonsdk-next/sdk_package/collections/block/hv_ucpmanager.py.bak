...
class UcpManager:
    ...
    def getAuthToken(self):
        ...
        try:
            ...
            raise Exception('Failed to establish a connection, please check the UCPA address and the connection.')
        ...
        if response.ok:
            ...
            raise Exception('UCPA login failed, please check the configuration.')
    ...
    def createUcpSystem(self,
        ...
        if system is not None and name != system['name']:
            raise Exception("UCP serial number is already in use, name change is not supported.")
    ...
    def getUcpSystem(self, serial):
        ...
        system = next((x for x in systems if str(x.get('serialNumber'))
                        == serial), None)
        
        if system is None:
            system = next((x for x in systems if str(x.get('name'))
                            == serial), None)
    ...
    def checkTaskStatus(self, taskId):
        ...
        while status == 'Running' or status == 'Pending':
            self.logger.writeInfo('{0} task with id {1} status is {2}'.format(name,
                                                                                  taskId, status))
            time.sleep(5)
            (status, name) = self.getTaskStatus(taskId)
        ...
        if status.lower() == 'failed':
            ...
            self.logger.writeDebug('{0} task with id {1} is failed.'.format(name, taskId))
    ...
    def deleteUcpSystem(self, resourceId):
        ...
        response = requests.delete(url, headers=self.getAuthToken(),
                                    verify=self.shouldVerifySslCertification)
    ...
    def updateUcpSystem(self, 
        ...
        response = requests.patch(url, headers=self.getAuthToken(),
                                    json=body, verify=self.shouldVerifySslCertification)
    ...
    def getAllLuns(self):
        ...
        urlPath = 'v2/storage/devices/{0}/volumes?fromLdevId=0&toLdevId=65000&refresh=false'.format(resourceId)
        url = self.getUrl(urlPath)
...
#############################################################################
    ...
    def addStorageSystem(
        ...
        if response.ok:
            ...
            self.logger.writeInfo('response err={}',
                                   response.json())
    ...
    def removeStorageSystem(self, storage_serial, ucp_serial):
        ...
        if attached :
            self.logger.writeInfo('Storage system is still attached to UCP, no removeStorageDevice')
            return
            
        # removeStorageDevice
        self.logger.writeInfo('Storage system is no lonager attached to any UCPs anymore, removeStorageDevice')
        resourceId = self.getStorageSystemResourceIdInISP()
