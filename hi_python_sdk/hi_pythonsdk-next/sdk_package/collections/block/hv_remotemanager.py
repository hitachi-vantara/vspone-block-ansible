...
try:
    from ansible_collections.hitachi.storage.plugins.module_utils.hv_htimanager import HtiPairType
...
class RemoteManager:
    ...
    def __init__(
        ...
        self.basedUrl = 'https://{0}:{1}'.format(webServiceIp,
                                                 webServicePort)
    ...
    def formatPairHTI(self, pair):
        ...
        if HAS_REPLICATION_STATUS:
            pair['Status'] = ReplicationStatus.fromValue(pair.get('Status',
                                                                  0)).name
    ...
    def formatPairTC(self, pair):
        ...
        if pair['Type'] == 'TRUE_COPY' or pair['Type'] == 'UR' \
                or pair['Type'] == 'GAD':
            if pair.get('Fence_Level') is not None \
                    and pair['Fence_Level'] != 'UNKNOWN':
                newpair['FenceLevel'] = pair['Fence_Level']
        if pair.get('FenceLevel') is not None and pair['FenceLevel'] \
                != 'UNKNOWN':
    ...
    def getUrl(self, urlPath):
        return '{0}/HitachiStorageManagementWebServices/{1}'.format(self.basedUrl,
                                                                    urlPath)
    ...
    def getTCPairsFromAll(self, rclist, lun):
        if lun is None or lun == -1:
            ...
            pairs = [self.formatPairTC(pair) for pair in rclist
                     if pair['Type'] == 5 and str(pair['PVol'])
                     == str(lun)]
        return pairs
    ...
    def getURPairsFromAll(self, rclist, lun):
        if lun is None or lun == -1:
            pairs = [self.formatPairUR(pair) for pair in rclist
                     if pair['Type'] == 6]
        else:
            pairs = [self.formatPairUR(pair) for pair in rclist
                     if pair['Type'] == 6 and str(pair['PVol'])
                     == str(lun)]
        return pairs
    ...
    def getURPairByPVol(
        ...
        rclist = obj
        pair = [pair for pair in rclist if str(pair['Type']) == str(6)
                and str(pair['PVol']) == str(lun)
                and (str(pair['SVolSerial']) == str(remote_serial)
    ...
    def getURPair(
        ...
        rclist = obj
        pair = [pair for pair in rclist if pair['Type'] == 6
                and str(pair['PVol']) == str(lun)
                and (str(pair['SVolSerial']) == str(remote_serial)
                     or str(pair['SVolSerial']) == str(0))
    ...
    def getConsistencyPairGroups(self, consistency_group_id=None):
        ...
        self.logger.writeEnterSDK(funcName)
        self.logger.writeParam('consistency_group_id={}',
    ...
    def createUR_tpi(
        ...
        url = \
            self.getUrl('TrueCopy/CreateUniversalReplicatorPairWithRemotePoolId'
                        )
        body = {
            'sessionId': self.sessionId,
            'serialNumber': self.serial,
    ...
    def createTC_tpi(
        ...
        if ctgId is not None:
            url = \
                self.getUrl('TrueCopy/CreateRemoteCloneWithCTGIdAndRemotePoolId'
                            )
            body['ctgId'] = ctgId
    ...
    def createGADPairVbox(
        ...
        url = \
            self.getUrl('TrueCopy/CreateVBOXGlobalActiveDevicePairWithCTG'
                        )
        body = {
            'sessionId': self.sessionId,
            'pVol': pVol,
    ...
    def createGADPairVboxCtgid(
        ...
        url = \
            self.getUrl('TrueCopy/CreateVBOXGlobalActiveDevicePairWithCTGId'
                        )
    ...
    def createGADPair_tpid(
        ...
        url = \
            self.getUrl('TrueCopy/CreateGlobalActiveDevicePairWithRemotePoolId'
                        )
        body = {
            'sessionId': self.sessionId,
            'serialNumber': self.serial,
    ...
    def splitGADPair(self, lun):
        ...
        body = {'sessionId': self.sessionId,
                'serialNumber': self.serial, 'pVol': lun}
